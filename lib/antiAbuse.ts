...
export async function checkIpAbuse(ip: string | null, windowMs = 24 * 60 * 60 * 1000, maxPerWindow = 5) {
  ...
  try {
    ...
    const events = await prismaAny.analyticsEvent.count({ where: { ip, createdAt: { gte: since } } });
...
export async function logEvent(event: string, userId?: string | null, ip?: string | null, meta?: any) {
  try {
    await prismaAny.analyticsEvent.create({ data: { event, userId, ip, meta: meta ? JSON.stringify(meta) : undefined } });
  } catch (e) {
